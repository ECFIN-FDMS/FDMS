...
class TestSplice(unittest.TestCase):
    ...
    def test_butt_splice(self):
        ...
        base_series_data = [pd.np.nan] * 4 + [
            714520000, pd.np.nan, pd.np.nan, 999350000, 1187470000, 1364510000, 1726780000, 2072000000,
        ...
        splice_series_data = [pd.np.nan] * 35 + [
            ...
            60.7046556, 62.91773177, pd.np.nan, pd.np.nan]
        splice_series_index = pd.Index([x for x in range(1960, 2026)], dtype='object')

        expected_base_series = pd.to_numeric(pd.Series(base_series_data, index=base_series_index, dtype='object',
                                         name=name))
        expected_splice_series = pd.to_numeric(pd.Series(splice_series_data, index=splice_series_index, dtype='object',
                                           name=splice_series.name))
        expected_backward_data = pd.np.concatenate([splice_series_data[:40], base_series_data[4:]])
        expected_forward_data = pd.np.concatenate([base_series_data[:18], splice_series_data[54:]])
        expected_both_data = pd.np.concatenate([splice_series_data[:40], base_series_data[4:18], splice_series_data[54:]])
